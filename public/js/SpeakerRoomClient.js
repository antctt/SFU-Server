...
/**
 * MiroTalk SFU - Speaker Room Client component
 * 
 * SPEAKER ROOM: Only participants with open microphones are visible
 * 
...
class RoomClient {
    ...
    async handleConsumer(id, type, stream, peer_name, peer_info) {
        ...
        switch (type) {
            ...
            case mediaType.screen:
                ...
                // SPEAKER ROOM: Only show video streams for peers with open microphones
                if (!this.shouldShowPeerInGrid(peer_info)) {
                    console.log('[handleConsumer] Peer video not shown in grid (mic off):', peer_name);
                    return;
                }
    ...
    setVideoOff(peer_info, remotePeer = false) {
        ...
        // SPEAKER ROOM: Apply initial visibility based on mic status
        this.updatePeerVisibilityForSpeakerRoom(peer_id, peer_audio);
    ...
    // ####################################################
    // SPEAKER ROOM - MIC-ONLY VISIBILITY
    // ####################################################

    shouldShowPeerInGrid(peer_info) {
        // In Speaker Room, we create all peer elements but control visibility
        // Always create elements - visibility will be controlled separately
        return true;
    }

    updatePeerVisibilityForSpeakerRoom(peer_id, micStatus) {
        // Update peer visibility based on microphone status in Speaker Room
        const videoElement = this.getId(peer_id + '__video');
        const videoOffElement = this.getId(peer_id + '__videoOff');
        
        console.log('[SpeakerRoom] Updating visibility for peer:', peer_id, 'micStatus:', micStatus);
        
        if (peer_id === this.peer_id) {
            // Hide local user completely with CSS class
            if (videoElement) {
                videoElement.classList.add('speaker-room-hide-local');
            }
            if (videoOffElement) {
                videoOffElement.classList.add('speaker-room-hide-local');
            }
        } else {
            // For remote users, show only if mic is on
            const shouldBeVisible = micStatus === true;
            if (videoElement) {
                videoElement.style.display = shouldBeVisible ? 'block' : 'none';
            }
            if (videoOffElement) {
                videoOffElement.style.display = shouldBeVisible ? 'block' : 'none';
            }
        }
        
        // Trigger aspect ratio recalculation since visibility changed
        if (videoElement || videoOffElement) {
            handleAspectRatio();
        }
    }

    handleSpeakerRoomMicChange(peer_id, micStatus) {
        // Handle microphone status changes for Speaker Room visibility
        console.log('[SpeakerRoom] Mic status changed for peer:', peer_id, 'micStatus:', micStatus);
        this.updatePeerVisibilityForSpeakerRoom(peer_id, micStatus);
    }

    hideLocalUserInSpeakerRoom() {
        // Simple CSS-based solution to hide all local user elements
        setTimeout(() => {
            const localVideoElements = document.querySelectorAll(`[id*="${this.peer_id}__video"]`);
            const localVideoOffElements = document.querySelectorAll(`[id*="${this.peer_id}__videoOff"]`);
            
            localVideoElements.forEach(el => el.classList.add('speaker-room-hide-local'));
            localVideoOffElements.forEach(el => el.classList.add('speaker-room-hide-local'));
            
            console.log('[SpeakerRoom] Hidden local user elements:', localVideoElements.length + localVideoOffElements.length);
        }, 1000); // Small delay to ensure elements are created
    }
    ...
    updatePeerInfo(peer_name, peer_id, type, status, emit = true, presenter = false) {
        if (emit) {
            ...
            switch (type) {
                case 'audio':
                    if (canUpdateMediaStatus) {
                        // Update peer info in our map first
                        if (this.peers.has(peer_id)) {
                            const peer_info = this.peers.get(peer_id);
                            peer_info.peer_audio = status;
                            this.peers.set(peer_id, peer_info);
                        }
                        this.setPeerAudio(peer_id, status);
                        // SPEAKER ROOM: Handle mic status change for visibility  
                        this.handleSpeakerRoomMicChange(peer_id, status);
                    }
                    break;
                case 'video':
                    break;
        ...
        try {
            ...
            switch (error.code) {
                case 'quota_not_enough':
                    this.msgPopup(
                        'warning',
                        'You have reached your quota limit for this demo account. Please consider upgrading for more features.',
                        6000,
                        'top'
                    );
                    break;
